...
const Discover = () => {
  ...
  return (
    <SafeAreaView>
      ...
      {/* Menu Container */}
      {isLoading ? (<View className="flex-1 items justify-center">
                    ...
                    <>
                    {
                      mainData?.map((data,i) => (
                        ...
                        imageSrc={data?.photo?.images?.medium?.url ?
                                  data.photo.images.medium.url : "https://static.vecteezy.com/system/resources/previews/005/337/799/original/icon-image-not-found-free-vector.jpg"} title={data?.name} location={data?.location_string} data={data}/>
                      ))
                    }
